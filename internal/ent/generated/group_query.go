...
func (gq *GroupQuery) FirstID(ctx context.Context) (id idx.ID, err error) {
	var ids []idx.ID
...
func (gq *GroupQuery) FirstIDX(ctx context.Context) idx.ID {
...
func (gq *GroupQuery) OnlyID(ctx context.Context) (id idx.ID, err error) {
	var ids []idx.ID
...
func (gq *GroupQuery) OnlyIDX(ctx context.Context) idx.ID {
...
func (gq *GroupQuery) IDs(ctx context.Context) (ids []idx.ID, err error) {
...
func (gq *GroupQuery) IDsX(ctx context.Context) []idx.ID {
	ids, err := gq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (gq *GroupQuery) loadUsers(ctx context.Context, query *UserQuery, nodes []*Group, init func(*Group), assign func(*Group, *User)) error {
	...
	byID := make(map[idx.ID]*Group)
	nids := make(map[idx.ID]map[*Group]struct{})
...
func (gq *GroupQuery) loadOwner(ctx context.Context, query *OrganizationQuery, nodes []*Group, init func(*Group), assign func(*Group, *Organization)) error {
	...
	for i := range nodes {
		...
		fk := *nodes[i].organization_groups
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "organization_groups" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
