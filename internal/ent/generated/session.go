...
type Session struct {
	...
	session_users *nanox.ID
	user_sessions *nanox.ID
...
func (*Session) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case session.FieldCreatedBy, session.FieldUpdatedBy, session.FieldType, session.FieldToken, session.FieldUserAgent, session.FieldIps:
	...
	for i := range columns {
		...
		case session.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullScanner); !ok {
				...
				s.session_users = new(nanox.ID)
				*s.session_users = *value.S.(*nanox.ID)
			}
		case session.ForeignKeys[1]:
			if value, ok := values[i].(*sql.NullScanner); !ok {
				return fmt.Errorf("unexpected type %T for field user_sessions", values[i])
			} else if value.Valid {
				s.user_sessions = new(nanox.ID)
				*s.user_sessions = *value.S.(*nanox.ID)
			}
		default:
