...
func AllowIfRelationshipExists() privacy.QueryMutationRule {
	return privacy.ContextQueryMutationRule(func(ctx context.Context) error {

		// TODO - get object and id
		allow, err := r.fgaClient.CheckDirectUser(ctx, memberRelation, fmt.Sprintf("%s:%s", "organization", "id"))
		if err != nil {
			return err
		}
...
// func OrganizationReadPolicyRyle() privacy.MutationRule {
// 	return privacy.OrganizationQueryRuleFunc(func(ctx context.Context, oq *generated.OrganizationQuery) error {}){

// 	}
// }

// // FilterOrganizationRule is a query/mutation rule that filters out entities that are not in the organization.
// func FilterOrganizationRule() privacy.QueryMutationRule {
// 	// OrganizationFilter is an interface to wrap WhereOrganizationID()
// 	// predicate that is used by both `Group` and `User` schemas.
// 	type OrganizationFilter interface {
// 		WhereOrganizationID(entql.IntP)
// 	}
...
// 		tf.WhereOrganizationID(entql.IntEQ(tid))
